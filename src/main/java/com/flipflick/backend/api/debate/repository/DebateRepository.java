...
public interface DebateRepository extends JpaRepository<Debate, Long> {
    ...
    @Query("""
        ...
        WHERE d.movie.tmdbId = :tmdbId AND d.isDeleted = false AND d.member.isDeleted = false
    ...
    @Query("""
        SELECT d FROM Debate d
        WHERE d.member.id = :memberId AND d.movie.tmdbId = :tmdbId AND d.isDeleted = false AND d.member.isDeleted = false
        ORDER BY d.createdAt DESC
    """)
    ...
    @Query("""
        SELECT d FROM Debate d
        WHERE d.id = :debateId AND d.isDeleted = false AND d.member.isDeleted = false
    """)
    ...
    @Query("""
        SELECT d FROM Debate d
        WHERE d.member.nickname = :nickname AND d.isDeleted = false AND d.member.isDeleted = false
        ORDER BY d.createdAt DESC
    """)
    ...
    @Query("""
        SELECT d FROM Debate d
        WHERE d.member.id = :memberId AND d.isDeleted = false AND d.member.isDeleted = false
        ORDER BY d.createdAt DESC
    """)
    ...
    @Query(value = """
        ...
        WHERE CAST(d.created_at AS DATE) <= :endDate AND d.is_deleted = false AND m.is_deleted = false
    ...
    @Query(value = """
        ...
        FROM debate d
        JOIN member m ON d.member_id = m.member_id
        WHERE CAST(d.created_at AS DATE) BETWEEN :startDate AND :endDate
          AND d.is_deleted = false AND m.is_deleted = false
        GROUP BY CAST(d.created_at AS DATE)
        ORDER BY CAST(d.created_at AS DATE)
    """, nativeQuery = true)
